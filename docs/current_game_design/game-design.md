...
- Игрок сначала щёлкает левой кнопкой по персонажу, затем правой — по цели; персонаж идёт к этой точке по построенному маршруту с обходом препятствий.
...
### 4.1. Прокладывание маршрута (pathfinding)

- **Поведение**: при правом клике по карте сервер автоматически строит маршрут от текущей позиции персонажа до цели с учётом препятствий (стены, ящики, сундуки, ресурсы). Персонаж движется по цепочке промежуточных точек (waypoints), а не по прямой линии.
- **Обход препятствий**: маршрут огибает все статические объекты. Персонаж не «упирается» в стену — он заранее обходит её по пути.
- **Игрок**: видит только результат — клик по точке, персонаж идёт к ней. Детали построения маршрута скрыты.
...
*Документ описывает состояние на момент реализации: движение с pathfinding, коллизии, препятствия, интерактивы (E), синхронизация нескольких игроков.*
